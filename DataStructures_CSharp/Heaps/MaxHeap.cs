...
namespace DataStructures_CSharp.Heaps {
    public class MaxHeap<T> : ICollection<T>, IEnumerable<T> where T : IComparable<T> {
        ...
        private void PrintAsTree(int index, string indent) {
            if (index >= Count) return;

            PrintAsTree(2 * index + 1, indent + "    ");

            Console.WriteLine(indent + values[index]);

            PrintAsTree(2 * index + 2, indent + "    ");
        }
